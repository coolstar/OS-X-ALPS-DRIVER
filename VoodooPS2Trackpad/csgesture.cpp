...
#pragma mark process touch thresholds
    ...
    if (sc->enableLegacyMode){
        if (sc->legacycount >= 3)
            abovethreshold = sc->legacycount;
    }
...
#pragma mark shift to last
    ...
    if (sc->enableLegacyMode)
        releasedfingers = sc->lastlegacycount - sc->legacycount;
...
void CSGesture::LegacyProcessGesture(csgesture_softc *sc){   
    if (sc->legacycount < 2 || (sc->legacyx[0] != -1 && sc->legacyx[1] != -1)){
        for (int i = 0; i < MAX_FINGERS; i++){
            sc->x[i] = -1;
            sc->y[i] = -1;
            sc->p[i] = -1;
    ...
    for (int i = 0; i < 2; i++){
        ...
        int min_target = -1;
        for (int j = 0; j < MAX_FINGERS; j++){
            int dist = distancesq(sc->lastx[j] - sc->legacyx[i], sc->lasty[j] - sc->legacyy[i]);
            int xdist = abs(sc->lastx[j] - sc->legacyx[i]);
            int ydist = abs(sc->lasty[j] - sc->legacyy[i]);
            if (dist < min_dist){
                if ((xdist < 50 && ydist < 50) || sc->lastx[j] == -1){
                    min_dist = dist;
                    min_target = j;
                }
            }
        }
        sc->x[min_target] = sc->legacyx[i];
        sc->y[min_target] = sc->legacyy[i];
        sc->p[min_target] = 10;
    }
...
void CSGesture::update_keyboard(uint8_t shiftKeys, uint8_t keyCodes[KBD_KEY_CODES]) {
    if (shiftKeys == KBD_LCONTROL_BIT){
        _pointingWrapper->updateKeyboard(keyCodes[0]);
    }
